...
def q_learning_policy(state):
    ...
    return np.ones(len(ACTIONS)) / len(ACTIONS)  # Uniform probability for all actions

def ppo_policy(state, agent_pos, ghosts):
    """PPO policy: Avoids ghosts by moving away from them."""
    action_probs = np.ones(len(ACTIONS)) * 0.1  # Small probability for all actions
    ...
    if best_action is not None:
        action_probs[best_action] = 0.7  # Favor moving away from ghosts

    return action_probs / action_probs.sum()  # Normalize probabilities

# ------------------------ Policy Blending ------------------------
def blend_policies(state, threat_index, agent_pos, ghosts):
    """
    Blends Q-learning and PPO policies based on the Threat Index.
    - Low threat → Q-learning (exploration) dominates.
    - High threat → PPO (survival) dominates.
    - Medium threat → Weighted blending of both.
    """
    q_action_probs = q_learning_policy(state)  # Exploration
    ppo_action_probs = ppo_policy(state, agent_pos, ghosts)  # Survival
    
    # Weighted blending based on Threat Index
    blended_probs = (1 - threat_index) * np.array(q_action_probs) + threat_index * np.array(ppo_action_probs)
    blended_probs /= blended_probs.sum()  # Normalize
    
    action = np.random.choice(len(ACTIONS), p=blended_probs)
    return ACTIONS[action]  # Return the action as a tuple (dx, dy)
...
for episode in range(EPISODES):
    ...
    while steps < 50:  # Max steps per episode
        ...
        action = blend_policies(agent_pos, threat_index, agent_pos, ghosts)  # Select action
